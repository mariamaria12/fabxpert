...
describe('Project list statusGroup filter (e2e)', () => {
  ...
  it('filters by visibleFor everyone and role', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set(authHeader(adminCookie))
      .send({
        visibleForRoleIds: [FIXTURES.employeeRole.id, FIXTURES.employeeRole2.id],
      })
      .expect(200);

    const afterMultiRole = await request(app.getHttpServer())
      .get('/projects')
      .query({ visibleFor: FIXTURES.employeeRole.id, pageSize: '50' })
      .set(authHeader(adminCookie))
      .expect(200);
