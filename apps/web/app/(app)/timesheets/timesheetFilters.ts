...
export function sortTimesheetsForExport(rows: TimesheetDto[]): TimesheetDto[] {
  return [...rows].sort((left, right) => {
    const dateCompare = new Date(left.workDate).getTime() - new Date(right.workDate).getTime();
    if (dateCompare !== 0) {
      return dateCompare;
    }

    const lastNameCompare = left.person.lastName.localeCompare(right.person.lastName, 'ro');
    if (lastNameCompare !== 0) {
      return lastNameCompare;
    }

    const firstNameCompare = left.person.firstName.localeCompare(right.person.firstName, 'ro');
